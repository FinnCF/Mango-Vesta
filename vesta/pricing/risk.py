...
class Risk:
    ...
    @staticmethod
    def calculate_current_values_binary(vsta, price, T, t, risk_free_rate, sigma, init_asset_weight, maint_asset_weight, init_liab_weight, maint_liab_weight, deposit_limit, slippage, supply):
        """
    ...
    @staticmethod
    def calculate_current_values_vanilla(vsta, price, T, t, risk_free_rate, sigma, init_asset_weight, maint_asset_weight, init_liab_weight, maint_liab_weight, deposit_limit, slippage, supply):
        """
        Calculate and return current values for given parameters.
        """
        leverage = 1 / (1 - init_asset_weight)
        ...
        # Analytic black scholes solutions
        current_value = vsta.analytical.vanilla_put_price(price, liq_price, T, t, risk_free_rate, sigma)
        current_delta = vsta.analytical.vanilla_put_delta(price, liq_price, T, t, risk_free_rate, sigma)
        current_gamma = vsta.analytical.vanilla_put_gamma(price, liq_price, T, t, risk_free_rate, sigma)
        current_theta = vsta.analytical.vanilla_put_theta(price, liq_price, T, t, risk_free_rate, sigma)
        current_speed = vsta.analytical.vanilla_put_speed(price, liq_price, T, t, risk_free_rate, sigma)
        current_vega = vsta.analytical.vanilla_put_vega(price, liq_price, T, t, risk_free_rate, sigma)

        current_maximum_potential_exposure = borrowed_quantity * current_value
        current_short_positions = current_delta * borrowed_value

        # Exposure collateral ratio as the monthly APR that should be charged (the fees required to cover the exposure to the protocol)
        current_exposure_coll_ratio = 100 * (current_maximum_potential_exposure / collateral_value)
        current_collateral_apr = current_exposure_coll_ratio * 12

        # Months to of fees to cover short:
        months_of_fees_to_cover_short = - (current_short_positions / current_maximum_potential_exposure)
        ...
        return pd.DataFrame({
            ...
            'Percent of Market Cap': [perc_of_mktcp],
            'Slippage %': [slippage],
            'Maximum Potential Exposure (Monthly Fees)': [current_maximum_potential_exposure],
            'Exposure Collateral Ratio (Monthly APR) (%)': [current_exposure_coll_ratio],
            'Collateral APR Required (%)': [current_collateral_apr],
            'Short Hedge Required': [current_short_positions],
            'Months of Fees To Cover Short': [months_of_fees_to_cover_short]
        })
